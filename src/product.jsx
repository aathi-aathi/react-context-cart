...
function Product(){
    const {products,increaseQuantity,decreaseQuantity,setProducts} = useContext(productContext)
    const totalAmount = products.reduce((acc, item) => acc + (item.quantity * item.price), 0);
    const handleRemove=(id)=>{
    const removed_product = products.filter((product)=> product.id !=id)
    setProducts(removed_product)
    }
   return(
      <>
